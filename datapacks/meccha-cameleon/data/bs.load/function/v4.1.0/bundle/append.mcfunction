...
execute unless data storage bs:data load.bundle run data modify storage bs:data load.bundle set value { link: "https://api.mcbookshelf.dev/download?version=4.1.0,modules=", message: {text:"[bundle]",click_event:{action:"open_url"},hover_event:{action:"show_text",value:"Click to open URL"}}, \
}
...
function bs.load:v4.1.0/bundle/concat with storage bs:data load.bundle
