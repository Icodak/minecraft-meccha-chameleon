# meccha:paintbrush/paint_cross   (macro: $(rid) $(cuboid) $(face) $(u) $(v) $(color) $(shadow)
...
$data modify storage meccha:rt cross set value {rid:"$(rid)",cuboid:"$(cuboid)",color:"$(color)",srcface:"$(face)",u0:$(u),v0:$(v),shadow:$(shadow)}
