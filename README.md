...
**Paint (test color/UV addressing)**
| Function | Args | Purpose |
|---|---|---|
| `dev/paint_pixel` | `cuboid,face,u,v,color` | Recolor one pixel by its tags (`color` = `"#RRGGBBAA"`). |
| `dev/paint_face` | `cuboid,face,color` | Recolor every pixel of one face. |
| `dev/paint_all` | `color` | Flood the whole rig with one color. |
...
| `dev/hide_overlays` | - | Make shading overlays transparent (inspect raw colors). |
| `dev/show_overlays` | - | Recompute all shading overlays now. |

**Info / inspect**
| Function | Args | Purpose |
|---|---|---|
| `dev/info` | - | Overview: registry status, pose, entity counts, last brush color. |
...
## Color-picker menu (Dialog UI)
A second, in-menu way to choose the brush color - **fully decorrelated** from
...
* `pause:false` + `after_action:"none"` keep the menu open so colors can be
...
tools/                           # Pillar 2 + 5 Python pipeline
  ...
  build_dialog.py                #   OKLCH color-picker -> dialog JSON
...
data/
  ...
  meccha/
    function/
      ...
      dialog/                    # color-picker Dialog handlers
...
* The brush color is **per-player**: a packed `0xRRGGBB` in the `meccha.color`
...
**Per-player** otherwise: roles/teams, brush color (`meccha.color`), dialog
