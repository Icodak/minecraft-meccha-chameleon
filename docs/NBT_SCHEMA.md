...
int2hex : { "0":"00", ... "255":"FF" }        # int -> hex byte (dialog colours)
...
multiplayer-safe source of truth for the active colour; `meccha:rt last_sample`
is only a global display convenience for `dev/info`.

## Colour-picker dialog (`meccha:dialog/*`)
